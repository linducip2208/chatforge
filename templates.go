...
{{template "sgroup" dict "id" "channels" "icon" "la-plug" "label" (T "sg_channels")}}
  ...
  <li class="nav-item"><a class="nav-link {{if eq .Active "meta_flows"}}active{{end}}" href="/admin/meta/flows"><i class="la la-stream la-lg"></i> WA Flows<span class="badge ms-1" style="background:#4F46E5;color:#fff;font-size:8px">META</span></a></li>
  <li class="nav-item"><a class="nav-link {{if eq .Active "meta_templates"}}active{{end}}" href="/admin/meta/templates"><i class="la la-file-alt la-lg"></i> Templates<span class="badge ms-1" style="background:#4F46E5;color:#fff;font-size:8px">META</span></a></li>
  <li class="nav-item"><a class="nav-link {{if eq .Active "meta_catalog"}}active{{end}}" href="/admin/meta/catalog"><i class="la la-tags la-lg"></i> Catalog<span class="badge ms-1" style="background:#4F46E5;color:#fff;font-size:8px">META</span></a></li>
  ...
  <div class="row">
    ...
    <div class="col-12 col-lg-8"><div class="card"><div class="card-header"><h4 class="card-header-title">{{T "adm_subscriptions"}}</h4></div>
      <div class="table-responsive"><table class="table table-sm card-table"><thead><tr><th>#</th><th>{{T "col_user"}}</th><th>{{T "adm_packages"}}</th><th>{{T "sub_expire"}}</th><th>{{T "col_action"}}</th></tr></thead><tbody>
        {{range .Subscriptions}}<tr><td>{{.ID}}</td><td>{{.User}}</td><td>{{.Pkg}}</td><td>{{.Expire}}</td><td><a class="btn btn-sm btn-white" href="/admin/subscriptions?edit={{.ID}}"><i class="la la-edit"></i></a><form method="post" action="/admin/subscriptions/delete" style="display:inline"><input type="hidden" name="id" value="{{.ID}}"><button class="btn btn-sm btn-danger"><i class="la la-trash"></i></button></form></td></tr>{{else}}<tr><td colspan="5" class="text-muted text-center">-</td></tr>{{end}}
      </tbody></table></div></div></div>
  {{if .EditID}}
  <div class="col-12 mt-3"><div class="card border-warning"><div class="card-header bg-warning bg-opacity-10"><h4 class="card-header-title"><i class="la la-edit me-1"></i> Edit Subscription #{{.EditID}}</h4></div><div class="card-body"><form method="post" action="/admin/subscriptions/edit"><input type="hidden" name="id" value="{{.EditID}}"><div class="row"><div class="col-md-6"><div class="form-group"><label>{{T "col_user"}}</label><select name="user_id" class="form-control">{{range $.Users}}<option value="{{.ID}}">{{.Name}} ({{.Email}})</option>{{end}}</select></div></div><div class="col-md-6"><div class="form-group"><label>{{T "adm_packages"}}</label><select name="package_id" class="form-control">{{range $.Packages}}<option value="{{.ID}}">{{.Name}}</option>{{end}}</select></div></div><div class="col-md-6"><div class="form-group"><label>{{T "sub_expire"}}</label><input name="expire" type="date" class="form-control" value="{{.EditExpire}}"></div></div><div class="col-md-6 pt-4"><button class="btn btn-primary lift"><i class="la la-save me-1"></i> Update</button> <a href="/admin/subscriptions" class="btn btn-white btn-sm ms-2">Cancel</a></div></div></form></div></div></div>
  {{end}}
  </div>
{{end}}
...
<div class="omni-wrap">
<div style="padding:8px 16px;border-bottom:1px solid #e5e7eb;display:flex;gap:8px;flex-shrink:0;align-items:center">
<select id="omniChannelFilter" onchange="omniLoad(this.value)" style="padding:6px 10px;border:1.5px solid #e5e7eb;border-radius:8px;font-size:12px;font-weight:600;color:#1e293b;background:#fff;cursor:pointer;font-family:inherit;min-width:140px">
...
function omniLoad(ch){var waChannels=['wa','meta','','private'];var usePro=ch&&!waChannels.includes(ch);var url=usePro?'/pro/omni/conversations?channel='+ch:'/omni/inbox';if(!usePro&&ch)url+='?channel='+ch;fetch(url).then(function(r){return r.json()}).then(function(data){var h='';if(!data||!data.length){var labels={wa:'WhatsApp',x:'X/Twitter',shopee:'Shopee',discord:'Discord',ig:'Instagram',fb:'Facebook',tg:'Telegram',meta:'Meta (IG+FB)'};var label=labels[ch]||ch||'All Channels';h='<div class=\"text-center py-5\"><div style=\"font-size:40px;margin-bottom:8px\">📭</div><div style=\"font-size:14px;color:#6B7280;font-weight:500\">Belum ada percakapan di '+label+'</div><div style=\"font-size:12px;color:#9CA3AF;margin-top:4px\">Kirim pesan dari channel untuk memulai, atau <a href=\"/channel-settings\" style=\"color:#4F46E5\">setting API Key</a> terlebih dahulu.</div></div>'}data.forEach(function(c){var ch2=c.channel||'wa';var isGrp=c.is_group?true:false;var colors={wa:'#25D366',meta:'#075E54',ig:'#E1306C',tg:'#2AABEE',fb:'#1877F2'};var cColor=isGrp?'#10B981':(colors[ch2]||'#6B7280');var cIcon=isGrp?'la-users':({wa:'la-whatsapp',meta:'la-cloud',ig:'la-instagram',tg:'la-telegram',fb:'la-facebook'}[ch2]||'la-comment');var cLabel=isGrp?'Grup':({wa:'WA',meta:'Meta',ig:'IG',tg:'TG',fb:'FB'}[ch2]||ch2.toUpperCase());var name=c.name||c.phone||'Unknown';var msg=(c.message||'').substring(0,60);var time=c.time||'';var initial=isGrp?'G':name.charAt(0).toUpperCase();h+='<div class=omni-item data-phone='+(c.phone||'')+' data-channel='+ch2+' data-name='+name.replace(/"/g,'&quot;')+' data-group='+(isGrp?'1':'0')+'><div class=omni-av style=background:'+cColor+'>'+initial+'</div><div class=omni-body><div class=omni-name>'+name+'</div><div class=omni-msg>'+msg+'</div></div><div class=omni-meta><div class=omni-time>'+time.substring(11,16)+'</div><div class=omni-ch style=background:'+cColor+'20;color:'+cColor+'><i class=\"la '+cIcon+'\"></i> '+cLabel+'</div></div></div>'});document.getElementById('omniList').innerHTML=h;omniUpdateBadges(data,ch)})}
...
function omniLoadMsgs(){if(!omniChatPhone)return;fetch('/inbox/messages?phone='+encodeURIComponent(omniChatPhone)).then(function(r){return r.json()}).then(function(msgs){var box=document.getElementById('omniMsgs');if(!msgs||!msgs.length){box.innerHTML='<div class=\"text-center text-muted py-4\">Belum ada pesan.</div>';box.scrollTop=box.scrollHeight;return}var html='';for(var i=0;i<msgs.length;i++){var m=msgs[i],side=m.type==='sent'?'flex-end':'flex-start';var cls=m.type==='sent'?'out':'in';var content=m.message||'';if(m.message&&m.message.startsWith('[media:')){var parts=m.message.substring(7,m.message.lastIndexOf(']')).split(':');var mtype=parts[0],murl=parts.slice(1).join(':');if(mtype==='image')content='<img src=\"'+murl+'\" style=\"max-width:200px;border-radius:8px;cursor:pointer\" onclick=\"window.open(this.src)\">';else if(mtype==='video')content='<div style=\"background:#f0f2f5;padding:8px;border-radius:8px\">🎥 Video</div>';else content='📎 '+parts.slice(1).join(':')}if(m.type==='received'&&m.sender_name)html+='<div class=\"d-flex w-100 mb-1\" style=\"justify-content:'+side+'\"><div class=\"omni-bubble '+cls+'\"><div style=\"font-size:11px;font-weight:600;color:#10B981;margin-bottom:2px\">'+m.sender_name+'</div><div>'+content+'<span class=\"omni-cht\">'+m.created.substring(11,16)+'</span></div></div></div>';else html+='<div class=\"d-flex w-100 mb-1\" style=\"justify-content:'+side+'\"><div class=\"omni-bubble '+cls+'\"><div>'+content+'<span class=\"omni-cht\">'+m.created.substring(11,16)+'</span></div></div></div>'}box.innerHTML=html;box.scrollTop=box.scrollHeight})}
...
omniLoad('');setInterval(function(){var s=document.getElementById('omniChannelFilter');omniLoad(s?s.value:'');if(omniChatPhone)omniLoadMsgs()},15000);
...
<div class="alert alert-info mb-3"><strong>Channel Configuration:</strong> WhatsApp → <a href="/wa">Account & QR</a> | Instagram/Facebook → <a href="/admin/meta">Meta Accounts</a> | Telegram → <a href="/settings">Settings</a> | X/Shopee/Discord/Line/Slack → add API keys below (BYOK).</div>
<div class="card"><div class="card-header d-flex justify-content-between"><h4 class="card-header-title">Custom API Keys</h4><button class="btn btn-primary btn-sm" onclick="document.getElementById('addForm').style.display='block'">+ Add Key</button></div><div class="card-body">
...
<div class="form-group"><label>Platform</label><select name="platform" class="form-control" required><option value="">-- Select Platform --</option><option value="x">X / Twitter</option><option value="shopee">Shopee</option><option value="discord">Discord</option><option value="line">Line</option><option value="slack">Slack</option></select></div>
<div class="form-group"><label>Name (Label)</label><input name="name" class="form-control" placeholder="My X Account" required></div>
<div class="row"><div class="col-6"><div class="form-group"><label>API Key / Consumer Key</label><input name="api_key" class="form-control"></div></div><div class="col-6"><div class="form-group"><label>API Secret / Consumer Secret</label><input name="api_secret" class="form-control"></div></div></div>
<div class="row"><div class="col-6"><div class="form-group"><label>Access Token</label><input name="token" class="form-control"></div></div><div class="col-6"><div class="form-group"><label>Token Secret / Shop ID</label><input name="token_secret" class="form-control"></div></div></div>
...
<table class="table table-sm"><thead><tr><th>Platform</th><th>Name</th><th>Keys</th><th>Actions</th></tr></thead>
<tbody>{{range .ChannelKeys}}<tr><td><span class="badge badge-soft-primary">{{.Platform}}</span></td><td>{{.Name}}</td><td><small>API: {{.APIKey|printf "%.12s"}}... | Token: {{.Token|printf "%.12s"}}...</small></td><td><form method="post" action="/channel-settings/delete" style="display:inline"><input type="hidden" name="id" value="{{.ID}}"><button class="btn btn-sm text-danger">Delete</button></form></td></tr>{{else}}<tr><td colspan="4" class="text-muted text-center py-3">No custom API keys yet. Click + Add Key to connect X, Shopee, Discord, Line, or Slack.</td></tr>{{end}}</tbody></table>
</div></div></div></div></div>
{{end}}`
